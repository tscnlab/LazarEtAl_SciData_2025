...
NmPerPixel = np.asarray(pandas.read_csv(os.path.dirname(os.path.realpath(__file__)) + '/oo_wls.csv', header=-1))
...
for afile in (df1["Original file"].str.split('.', expand=True)[0]+'.csv').iteritems():
    ...
    uJPerNm = uJPerPixel[0]/np.transpose(NmPerPixel[0])
    uJPerCm2PerNm = uJPerNm/sensorAreaCm2
    uWPerCm2PerNm = uJPerCm2PerNm/integrationTime
    # Resample
    uWPerCm2PerNm = np.interp(np.arange(380, 781), np.asarray(specdf["Wavelengths (nm)"]), uWPerCm2PerNm)
    uWPerCm2PerNm = np.nan_to_num(uWPerCm2PerNm)
